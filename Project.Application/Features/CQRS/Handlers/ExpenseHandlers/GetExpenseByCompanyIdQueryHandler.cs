...
{
    public class GetExpenseByCompanyIdQueryHandler
    {
        private readonly IUnitOfWork unitOfWork;

        public GetExpenseByCompanyIdQueryHandler(IUnitOfWork unitOfWork)
        {
        ...
        public async Task<List<GetExpenseWithEmployeesQueryResult>> Handle(GetExpenseByCompanyIdQuery query)
        {
            var values = await unitOfWork.expenseRepository.GetExpenseWithEmployeeByCompanyId(query.Id);

            return values.Select(x=> new GetExpenseWithEmployeesQueryResult
            {
                Id = x.Id,
